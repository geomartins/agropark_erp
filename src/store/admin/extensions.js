...
const actions = {
    async create({commit, state},instance){
        try{
            ...
            let extension = new Extension(name,description,category ,id);
    ...
    async fetch({commit, state}, type){
        ...
        let unsubscribe = new Extension().fetch(type, (datas, err) => {
            if(err){ 
                snackbar('warning',err.message);
    ...
    async dependencies({commit, state}){
        try{
            ...
            await new Extension().dependencies((categories) => {
    ...
    async search({commit,dispatch, state},value){
        ...
        Extension.search(value).then((datas) => {
            commit('UPDATE_DATA',datas);
        }).catch(err => {
            snackbar('warning',err.message);
        });
    ...
    async delete({commit, state},id){
        ...
        x.onOk(()=> {
            console.log(id);
            Extension.deleteById(id).then().catch(err => {
                snackbar('warning',err.message);
            });
        })
    },
