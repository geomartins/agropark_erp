<template>
    <div class="q-pa-md">
        <q-table flat grid separator="cell" :data="datas" :columns="columns" row-key="id"  :loading="$store.state.extension_notifiers.loading"  :table-header-style="{textTransform: 'uppercase'}" :pagination="initialPagination">
...
export default Vue.extend({
    name: "ExtensionNotifiersListView",
    ...
    computed: {
      datas: {
           get: function() { return this.$store.getters["extension_notifiers/fetchData"]; },
           set: function(value){ this.$store.commit('extension_notifiers/UPDATE_DATA',value); }
      },
