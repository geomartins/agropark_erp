...
export default {
  ...
  methods: {
    ...
    editItem(payload) {
      ...
      this.$store.commit("users_details/UPDATE_EDIT_BANK_FORM_DATA", payload);
      this.$store.commit(
        "admin_layout/UPDATE_COMPONENT_NAME",
        "app-users-bank-update-form"
      );
    ...
    showCreateForm() {
      ...
      this.$store.commit(
        "admin_layout/UPDATE_COMPONENT_NAME",
        "app-users-bank-create-form"
      );
      this.$store.commit("admin_layout/UPDATE_RIGHT_DRAWER_OPEN", true);
    },
    async showBanks() {
      this.$store.dispatch("users_details/fetchBanks", this);
    }
  }
};
</script>
