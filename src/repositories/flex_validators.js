class FlexValidators{
    ...
    check(objs){
        for(var key in objs){ //{ description: 'required|isArray'}
    ...
    checkValidator(formDataKey, validators){
        for( var validator of validators){ //['required','isArray']
    ...
    notNull(formDataKey, rules){
        if(this.formData[formDataKey] ===  null || this.formData[formDataKey] === undefined || this.formData[formDataKey] === "" || this.formData[formDataKey].length < 1 ){
            this.errors.push(`[ ${formDataKey} ] field cannot be null or empty`)
        }
    }

    email(formDataKey, rules){
